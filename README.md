...
The ETL data pipeline was built using Postgres, Apache Airflow, AWS Redshift, and S3. This web app was built using Flask, Plotly Dash and Mapbox and is currently hosted [here](https://www.ofbennett.com). Throughout, I make extensive use of Docker and Docker-Compose to manage the various deployment environments for the different tools and databases.
...
Airflow is a great fit for this type of project as well. It makes scheduling tasks very simple, and can manage dependencies between different tasks in an arbitrarily complex Directed Acyclic Graph (DAG). This allows multiple parts of your pipeline to run simultaneously in a carefully coordinated way and allows precise monitoring of the pipeline across multiple runs. The Airflow UI makes visualisation of your pipelines very simple and elegant as well.

## The Data Sources

The majority of the data comes from a large volume (~100GB) of anonymised GP prescription records which have been released publicly by NHS digital over the course of the last 10 years. The data can be downloaded from their website [here](https://digital.nhs.uk/data-and-information/publications/statistical/practice-level-prescribing-data) and a detailed description of what the data contains can be found [here](https://digital.nhs.uk/data-and-information/areas-of-interest/prescribing/practice-level-prescribing-in-england-a-summary/practice-level-prescribing-data-more-information). This public sector information is published and made available under the Open Government Licence v3.0. Prescription data is released once a month and each monthly release consists of a CSV file containing a little over 9 million rows.
...
Finally, a free and open source API called Postcodes.io was used to obtain latitude and longitude coordinates (as well as other location metadata) of the GP practices in the dataset to make plotting them easier.
...
In order to enable SSL/TSL and serve the app securely over https I setup a reasonably simple two step deployment process using Let's Encrypt as a certificate authority. This process proceeded like this:
...
If you don't already have them, you will need to install Docker and install Docker-Compose on your local machine. Then create a suitable Python virtual environment.

```
$ git clone https://github.com/sgchandana29/nhs-prescribing-etl-pipeline.git
...
Next you need to get the location metadata for each GP practice in the GP practice dataset. This uses the free API provided by Postcodes.io. I wrote a Python script which does this. Make sure the file names and paths at the top of this script are correct for your local setup and run it:
...
The Postgres data warehouse is now populated. To run some queries I wrote a Python script which will do this for you and save the results into the webapp for later visualisation. Run:
...
You will also need the aws cli installed and to copy your AWS credentials into your .aws/credentials file.
...
- You can deploy it on a remote Linux virtual machine using something like DigitalOcean or AWS Lightsail and run a production grade server. This will make it available on the public internet.
...
1. Spin up a Linux instance using a cloud provider like DigitalOcean or AWS Lightsail.
...
```
12. Generate strong Diffie-Hellman coefficients for added security
```
...
- So far the cloud based ETL pipeline has only been run with a year's worth of data to keep AWS costs down. After some minor adjustments the plan is to run it next with all 10 year's worth of data.
...
- Develop "Infrastructure as Code" for the tech stack used in the project. Could use CloudFormation, Terraform, or simple boto3 AWS SDK scripts.
...
Sai Chandana Gamini is a Data Engineer with over 5 years of experience building cloud-based data platforms, distributed ETL pipelines, and AI-enabled analytics solutions. Specializing in Python, AWS, Airflow, and PySpark, Sai focuses on developing scalable batch and near real-time processing systems with a strong emphasis on performance optimization and data reliability.
